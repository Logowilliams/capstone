...
```{r}
#load data
df <- read_csv(here("data", "R12512595_SL140.csv"))
```
...
df%>%
  ...
  select(GEOID, pop, wht, unemp, wrkfrc, pov1, pov2, pov3, com_tot, no_com, no_int, dial_up, edu_tot, 
         brdbnd, pct_min, pct_unemp, pct_pov, pct_nocom, nohs, nohsgrad, hsgrad, colgrad) -> df
```

```{r}
#load spatial data

stl_cnty <- county_subdivisions(state = "MO", county = 510, cb = FALSE)
stl <- county_subdivisions(state = "MO", county = 189, cb = FALSE)
stclr <- county_subdivisions(state = "IL", county = 163, cb = FALSE)
